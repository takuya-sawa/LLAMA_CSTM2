...
#include "ggml-cuda/sgemm-f32-fallback.cuh"
...
static void ggml_cuda_op_mul_mat_cublas(
    ...
    if (supports_bf16 && src0->type == GGML_TYPE_BF16 && ggml_is_contiguous(src0) && row_diff == src0->ne[1]) {
        ...
        cublasHandle_t cbh = ctx.cublas_handle(id);
        if (cbh) {
            CUBLAS_CHECK(cublasSetStream(cbh, stream));
            CUBLAS_CHECK(
                cublasSgemm(cbh, CUBLAS_OP_T, CUBLAS_OP_N,
                        row_diff, src1_ncols, ne10,
                        &alpha, src0_ddf_i,  ne00,
                                src1_ddf1_i, ne10,
                        &beta,  dst_dd_i,    ldc));
        } else {
            // cuBLAS unavailable ? use tiled fallback kernel (e.g., Blackwell consumer GPUs)
            ggml_cuda_sgemm_f32_fallback(
                src0_ddf_i,  ne00, /*transA=*/true,
                src1_ddf1_i, ne10, /*transB=*/false,
                dst_dd_i,    ldc,
...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    get_rows_cuda(src1->data, src1->type, ids_to_sorted, GGML_TYPE_I32, src1_sorted.ptr, type_src1_sorted,
        ne10, nb11, nb12, nb13,
    ...
    get_rows_cuda(dst_sorted.ptr, type_dst_sorted, ids_from_sorted, GGML_TYPE_I32, dst->data, dst->type,
...
static bool ggml_backend_cuda_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_OUT_PROD:
            // dst and src1 must be F32; src0 may be F32 or any quantized type
            // (quantized src0 is dequantized to a temp F32 buffer inside the kernel)
            ...
            return ggml_get_to_fp32_cuda(op->src[0]->type) != nullptr;
        case GGML_OP_GET_ROWS:
            {

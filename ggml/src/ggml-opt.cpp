...
struct ggml_opt_context {
    ...
    // extended fields for finetune API compatibility
    bool    momenta_on_host = false;
...
static void ggml_opt_build(ggml_opt_context_t opt_ctx) {
    ...
    // For non-static (dynamic) graphs, reset param grad_accs at the START of each gradient-accumulation
    // window (opt_i == 0). Without this, grad_accs retain gradient values from the previous window and
    // accumulate across windows/epochs, causing incorrect (sum of many past gradients) inputs to AdamW.
    //
    // We only reset when opt_i == 0 so that intentional within-window accumulation (opt_period > 1) is
    // preserved. For opt_period == 1, opt_i is always 0 so every step is reset correctly.
    //
    // The very first call is exempt (buf_static == nullptr) because ggml_graph_reset is called after that
    // initial allocation, which already zeros grad_accs and sets the LOSS grad_acc to 1.0.
    if (opt_ctx->buf_static && !opt_ctx->static_graphs && !opt_ctx->grad_accs.empty() &&
            opt_ctx->opt_i == 0) {
        const int n = std::min((int)opt_ctx->grad_accs.size(), (int)opt_ctx->gf->n_nodes);
        for (int i = 0; i < n; i++) {
            struct ggml_tensor * ga = opt_ctx->grad_accs[i];
            if (!ga) {
                continue;
            }
            if (opt_ctx->gf->nodes[i]->flags & GGML_TENSOR_FLAG_LOSS) {
                // Loss grad_acc must be 1.0 (d loss / d loss = 1)
                const float onef = 1.0f;
                ggml_backend_tensor_set(ga, &onef, 0, sizeof(float));
            } else {
                ggml_set_zero(ga);
            }
        }
...
bool ggml_opt_result_nan_detected(ggml_opt_result_t result) {
    return result ? result->nan_detected : false;
}

int ggml_opt_result_nan_grad_count(ggml_opt_result_t result) {
    return result ? result->nan_grad_count : 0;
}

// ====== Extended context API (finetune) ======

int64_t ggml_opt_get_iter(ggml_opt_context_t opt_ctx) {
    return opt_ctx ? opt_ctx->iter : 0;
}

void ggml_opt_set_momenta_on_host(ggml_opt_context_t opt_ctx, bool on_host) {
    if (opt_ctx) { opt_ctx->momenta_on_host = on_host; }
}

void ggml_opt_set_grad_clip(ggml_opt_context_t opt_ctx, float max_grad) {
    if (opt_ctx) { opt_ctx->grad_clip = max_grad; }
}

float ggml_opt_get_grad_clip(ggml_opt_context_t opt_ctx) {
    return opt_ctx ? opt_ctx->grad_clip : 0.0f;
}

void ggml_opt_set_grad_norm_clip(ggml_opt_context_t opt_ctx, float max_norm) {
    if (opt_ctx) { opt_ctx->grad_norm_clip = max_norm; }
}

float ggml_opt_get_grad_norm_clip(ggml_opt_context_t opt_ctx) {
    return opt_ctx ? opt_ctx->grad_norm_clip : 0.0f;
}

void ggml_opt_set_loss_scale(ggml_opt_context_t opt_ctx, float scale) {
    if (opt_ctx) { opt_ctx->loss_scale = scale; }
}

float ggml_opt_get_loss_scale(ggml_opt_context_t opt_ctx) {
    return opt_ctx ? opt_ctx->loss_scale : 1.0f;
}

void ggml_opt_set_warmup_steps(ggml_opt_context_t opt_ctx, int64_t warmup_steps) {
    if (opt_ctx) { opt_ctx->warmup_steps = warmup_steps; }
}

void ggml_opt_set_total_steps(ggml_opt_context_t opt_ctx, int64_t total_steps) {
    if (opt_ctx) { opt_ctx->total_steps = total_steps; }
}

void ggml_opt_set_min_lr_ratio(ggml_opt_context_t opt_ctx, float ratio) {
    if (opt_ctx) { opt_ctx->min_lr_ratio = ratio; }
}

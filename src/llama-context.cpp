...
void llama_context::sched_reserve() {
    ...
    // Allocate sched with 4x capacity so backward graphs (used in training) can also be scheduled.
    // For inference-only usage the extra capacity is unused but causes no functional overhead.
    const size_t max_nodes_sched = max_nodes * 4;
    sched.reset(ggml_backend_sched_new(backend_ptrs.data(), backend_buft.data(), backend_ptrs.size(), max_nodes_sched, cparams.pipeline_parallel, cparams.op_offload));
...
void llama_context::opt_init(struct llama_model * model, struct llama_opt_params lopt_params) {
  ...
  //llama_set_param(model->tok_embd,        param_filter, param_filter_ud); // FIXME
    ...
    // Also mark LoRA adapter tensors as trainable parameters.
    // LoRA tensors (*.lora_a / *.lora_b) live in the adapter, not in model->layers,
    // so they must be registered separately for gradient tracking.
    if (loras) {
        for (auto & [adapter, scale] : *loras) {
            for (auto & [name, weight] : adapter->ab_map) {
    ...
    for (uint32_t pos_ctx = 0; pos_ctx < n_ctx; pos_ctx += n_batch) {
        ...
        do {
            ...
            struct ggml_context * ctx_compute_opt;
            {
                ...
                // backward/opt graphs are each up to 4x the forward size (gb_grad + gb_opt = 2 graphs)
                const size_t size_gf_bwd = size_gf * 4;
                const size_t size_meta = 16*size_gf*ggml_tensor_overhead() + 2*ggml_graph_overhead_custom(size_gf_bwd, /*grads = */ true);
            ...
            res->set_inputs(&ubatch);
            {
                ...
                for (uint32_t pos_ubatch = 0; pos_ubatch < n_ubatch; ++pos_ubatch) {
                    ...
                    if (labels_sparse[ilabel] < 0) {
                        continue; // masked token, skip (no loss contribution)
                    }
...
// Return ggml optimizer context associated with given llama_context (may be NULL)
ggml_opt_context_t llama_opt_context_get(struct llama_context * ctx) {
    if (!ctx) return nullptr;
    return ctx->get_opt_ctx();
}

ggml_opt_context_t llama_context::get_opt_ctx() const {
    return opt_ctx;
}
